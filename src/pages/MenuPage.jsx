...
export default function MenuPage() {
  ...
  const tagline = settings?.tagline_ar || 'مطعم مأكولات بحرية فاخر'
  ...
  return (
    <div onClick={onClick} className="min-h-screen">
      ...
      <header className="sticky top-0 z-40 border-b border-accent/10 bg-surface-2/85 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl items-center gap-3 px-4 py-3 sm:px-4 sm:py-3 md:px-6 md:py-4">
          ...
          <span className="hidden rounded-full bg-accent/5 px-3 py-1 text-[11px] text-text/60 md:block">
            قائمة الطعام
          </span>
        </div>
      </header>
      ...
      <section className="relative overflow-hidden px-4 pb-2 pt-6 sm:px-4 md:px-6">
        <motion.div
          ...
          className="relative mx-auto max-w-5xl overflow-hidden rounded-2xl border border-accent/15 bg-gradient-to-br from-white/60 to-white/20 p-5 text-center sm:p-8 md:rounded-3xl md:p-10"
        >
      ...
      <main className="mx-auto max-w-5xl px-4 pb-16 pt-6 sm:px-4 md:px-6 md:pb-20">
        {error && (
          <div className="rounded-xl border border-red-500/30 bg-red-100 p-4 text-center text-sm text-red-700">
          ...
          <div className="mt-4 flex items-center justify-center gap-3">
            {settings?.phone && (
              <a
                ...
                className="grid h-11 w-11 place-items-center rounded-full bg-accent/5 text-accent transition hover:bg-accent hover:text-surface-2"
              >
                <Phone size={18} />
              </a>
            )}
        ...
        <div key={g}>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="flex gap-3 rounded-2xl border border-accent/10 bg-white/40 p-3">
